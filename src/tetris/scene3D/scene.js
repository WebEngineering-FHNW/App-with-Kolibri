...
const registerForMouseAndTouch = (scene, consumeEvents=false) => {
    ...
    const track = evt => {
        ...
        coords.style.setProperty('--coords-rotate-x', String(Number(lastCoordXrotate)-diffVertical));
        coords.style.setProperty('--coords-rotate-y', String(Number(lastCoordYrotate)+diffHorizontal));
    };
