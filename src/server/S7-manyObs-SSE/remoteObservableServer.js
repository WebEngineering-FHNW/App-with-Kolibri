...
/**
 * the central dynamic key-value observable that
 * binds any value updates to the event stream.
 * Since we always set a new key/value object, there are never identical values.
 * @type { IObservable<{key: String, value: *}> }
 */
const keyValueObservable = Observable( {key:"",value:""} );

/**
 * the "map" that keeps the latest known values of any key that was ever set and not removed.
 * To some extent, SSE holds the latest updates available for its clients but not for long,
 * which is why we need this.
 * @warning **if keys are never removed from this map this can lead to memory leaks**
...
const handleSSE = (req, res) => {
    ...
    const sendText = (newKeyValuePair, _oldKeyValuePair, removeMe) => {  // how to listen for text changes
